...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.Dropdown(id='site-dropdown', options =[{'label': 'All Sites', 'value': 'ALL'},{'label': 'CCAFS LC-40', 'value': 'CCAFS LC-40'}, {'label': 'VAFB SLC-4E', 'value':'VAFB SLC-4E'}, {'label': 'KSC LC-39A', 'value':'KSC LC-39A'}, {'label': 'CCAFS SLC-40', 'value': 'CCAFS SLC-40'}], value='ALL', placeholder="Please select a Launch Site", searchable=True),
                                html.Br(),
...
def get_scatter_chart(entered_site, payload_range):
    ...
    filtered_df_all = spacex_df[(payload_range[0]<=spacex_df['Payload Mass (kg)']) & (spacex_df['Payload Mass (kg)']<= payload_range[1])]
    if entered_site == 'ALL':
